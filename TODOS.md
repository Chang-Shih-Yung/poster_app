...
## P1 — 抽 PosterFieldset 共用元件，PosterForm + DraftCard 不再各寫一份

**What:** 目前單張新建 (`PosterForm.tsx` ~1100 行) 跟批量 (`DraftCard.tsx` ~700 行) 各自寫一份 30 個欄位的 JSX，因此每次 spec 改一條就要兩處同步、容易漏掉（過去已撞過好幾次：批量還是單張上傳、限量欄位漏批量、組合欄位 placeholder 文案不同步等）。

**Why:** Source of truth 應該是「一張海報的表單長什麼樣」。批量只是把這張表單套到 N 個 draft 上、加一個「套用到全部卡片」bulk-fill 區。User 心智模型也是這樣 — 表單一份，被使用在兩個情境（單張 + 批量）。

**目標架構：**

```
PosterFieldset（純 controlled 元件，吃 values + onChange）
  ├─ /posters/new   : 包 1 個 fieldset + 送出按鈕
  └─ /posters/batch : 包 N 個 fieldset（一張 draft 一個）+ 套用區（套用邏輯不變）
```

**File scope:**
- 新 `components/PosterFieldset.tsx` — 純 controlled，吃 `{ values, onChange, errors?, posterId, size, works, ...callbacks }`，render 全部 spec 欄位（含主圖 picker、海報發行組合、宣傳圖 gallery、是否限量、是否有工藝、是否公開）
- `app/posters/new/PosterForm.tsx` — 重構成「fieldset + 送出按鈕」薄殼，drop RHF 改用 plain useState（DraftCard 已是 plain state，跨用同一份元件比較順）；validation 抽成 `lib/validatePosterValues.ts`，submit 時呼叫
- `app/posters/batch/DraftCard.tsx` — 上半縮圖 + 狀態 chip 不動，下半 expand 區塊改 render `<PosterFieldset values={draft} onChange={...} />`
- `app/posters/batch/_shared.ts` — `DraftPoster` 跟 `PosterFieldsetValues` 對齊（理想上同一個 type）
- `app/posters/batch/BatchImport.tsx` — 套用區 + submit pipeline 不動

**Effort:** L (~3-4 小時：寫 PosterFieldset + 抽 validate fn + 改寫 PosterForm + 改寫 DraftCard + 跑 tsc + 測四個流程：單張新建/編輯、批量新建、套用區)
**Priority:** P1（架構債，不修下次 spec 改動還是會漏）
**Depends on:** 無
**Reference:** 對話 commit `4e8819f` 之後 user 明確要求這個重構
