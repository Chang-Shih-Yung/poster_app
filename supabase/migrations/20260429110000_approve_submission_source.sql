-- ═══════════════════════════════════════════════════════════════════════════
-- approve_submission: stamp source = 'submission' on the poster row.
--
-- Follow-up to 20260429100000_enum_expansion_and_source.sql which added
-- `posters.source` (admin | submission). The backfill there matched
-- existing approved-via-RPC rows by `submissions.created_poster_id`,
-- but new approvals were still landing with source = NULL because the
-- RPC wasn't aware of the column. This migration teaches the RPC to
-- write `source = 'submission'` on insert so going forward every poster
-- has a known provenance.
--
-- The function body is a verbatim copy of the previous version with
-- two columns added to the INSERT (source + status was already there;
-- we slot source next to status for readability). No other behavioral
-- changes.
-- ═══════════════════════════════════════════════════════════════════════════
