...
export default async function EditPosterPage({
  ...
  const [{ data: poster }, { data: works }] = await Promise.all([
    supabase
      .from("posters")
      .select(
        "id, work_id, parent_group_id, poster_name, region, poster_release_type, size_type, channel_category, channel_name, is_exclusive, exclusive_name, material_type, version_label, source_url, source_note, is_placeholder, poster_url, thumbnail_url"
      )
      .eq("id", id)
      .single(),
